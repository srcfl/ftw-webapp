 ...
 * at 812 of an 874-point screen while the inset still claims 34px. Every
 * other row must keep the normal fixed shell.
 */

import { describe, it, expect } from 'vitest'
import { reservedByTheOs } from './reserved'

describe('the short installed viewport', () => {
  it('finds an installed iOS app laid out short of the screen', () => {
    expect(
      reservedByTheOs({ standalone: true, innerHeight: 812, screenHeight: 874, insetBottom: 34 })
    ).toBe(true)
  })

  it('is us, in a Safari tab — the window is the screen and the inset is real', () => {
    expect(
      reservedByTheOs({ standalone: false, innerHeight: 874, screenHeight: 874, insetBottom: 34 })
    ).toBe(false)
  })
  ...
  it('is us, on an installed app whose window really does reach the bottom', () => {
    ...
    expect(
      reservedByTheOs({ standalone: true, innerHeight: 874, screenHeight: 874, insetBottom: 34 })
    ).toBe(false)
  })

  it('is nobody, on a screen with no indicator at all', () => {
    expect(
      reservedByTheOs({ standalone: true, innerHeight: 800, screenHeight: 874, insetBottom: 0 })
    ).toBe(false)
  })
  ...
  it('is us, anywhere the platform does not answer the question', () => {
    ...
    expect(
      reservedByTheOs({ standalone: undefined, innerHeight: 812, screenHeight: 874, insetBottom: 34 })
    ).toBe(false)
  })
})
